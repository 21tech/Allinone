...
public class QueryGradeServiceImpl implements QueryGradeService {
	 ...
	 */
	@Override
	public List<XsCjAllTdkchView> getCurrentGradeByXh(String xh) throws NullUserGradeException, DBException {
		List<Object> rs;
			...
			if (rs != null) {
				if (rs.size() > 0) {
					for (int i = 0; i < rs.size(); i++) {
						...
						tmp.setXf(objClass[9] != null && !objClass[9].equals("")
								? (/*
									 * objClass[9].toString().length()==1?
									 * objClass[9].toString()+".0":
									 */objClass[9].toString()) : null);
						...
						case 1:
							tmp.setZwksxq(tmp.getZxjxjhh().substring(0, 4) + "年秋季");
							tmp.setYwksxq(tmp.getZxjxjhh().substring(0, 4) + "Fall");
							break;
						case 2:
							tmp.setZwksxq(tmp.getZxjxjhh().substring(5, 9) + "年春季");
							tmp.setYwksxq(tmp.getZxjxjhh().substring(5, 9) + "Spring");
							break;
						case 3:
							tmp.setZwksxq(tmp.getZxjxjhh().substring(5, 9) + "年夏季");
